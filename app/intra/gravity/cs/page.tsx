...
export default function GravityCsPage() {
    ...
    const st = (s: string) => ({ pending: { l: "미답변", c: "bg-amber-50 text-amber-700" }, answered: { l: "완료", c: "bg-emerald-50 text-emerald-700" }, closed: { l: "종료", c: "bg-neutral-100 text-neutral-500" } }[s] ?? { l: s, c: "bg-neutral-100 text-neutral-500" });
        ...
        <div>
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm text-neutral-400 mt-0.5">Brand Gravity™ 문의 · CS 관리</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
                {[{ l: "전체", v: inquiries.length, c: "" }, { l: "미답변", v: inquiries.filter(i => i.status === "pending").length, c: "text-amber-600" }, { l: "완료", v: inquiries.filter(i => i.status === "answered").length, c: "text-emerald-600" }].map(({ l, v, c }) => (
            ...
            {loading ? (
                ...
                <div className="border border-neutral-200 rounded-lg overflow-hidden">
                    <table className="w-full text-sm">
                        <thead><tr className="bg-neutral-50 text-left">{["이름","이메일","제목","상태","일시"].map(h => <th key={h} className="px-4 py-3 font-semibold text-neutral-500">{h}</th>)}</tr></thead>
                        <tbody>
