    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-xl font-bold">전체 일정 관리</h1>
                    <p className="text-xs text-neutral-400 mt-0.5">전사 일정 등록 및 관리 · 공개 일정은 퍼블릭 사이트에도 노출</p>
                </div>
            ...
            {viewMode === 'calendar' ? (
                <div className="grid grid-cols-[1fr_320px] gap-5">
                    ...
                    <div className="border border-neutral-200 bg-white rounded">
                        ...
                        <div className="grid grid-cols-7">
                            {DAYS.map(d => (
                                <div key={d} className="px-2 py-2 text-center text-xs font-medium text-neutral-400 border-b border-neutral-100">{d}</div>
                            ))}
                            {cells.map((day, i) => {
                                    ...
                                    <button key={i} onClick={() => setSelectedDate(dateStr)}
                                        ...
                                        <div className="mt-0.5 space-y-0.5">
                                            {dayEvents.slice(0, 3).map(ev => (
                                                <div key={ev.id} className={clsx("text-[11px] px-1 py-0.5 rounded truncate flex items-center gap-0.5", typeColors[ev.type])}>
                                                    {ev.visibility === 'public' && <Globe className="h-2 w-2 shrink-0" />}
                                                    {ev.title}
                                                </div>
                                            ))}
                                            {dayEvents.length > 3 && <p className="text-[11px] text-neutral-400 px-1">+{dayEvents.length - 3}</p>}
                                        </div>
                                    </button>
                                );
                            })}
                    ...
                    <div className="border border-neutral-200 bg-white rounded self-start">
                        <div className="px-4 py-3 border-b border-neutral-100 flex items-center justify-between">
                            ...
                            <button onClick={() => openEditor()} className="text-xs text-neutral-400 hover:text-neutral-900">+ 추가</button>
                        </div>
                                ...
                                {selectedEvents.map(ev => (
                                    <li key={ev.id} className="px-4 py-3 group">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className={clsx("text-[11px] px-1.5 py-0.5 rounded font-medium", typeColors[ev.type])}>{ev.type}</span>
                                            <span className="text-xs text-neutral-400 flex items-center gap-0.5"><Clock className="h-2.5 w-2.5" /> {ev.time}{ev.endTime ? `–${ev.endTime}` : ''}</span>
                                        ...
                                        <div className="hidden group-hover:flex gap-1 mt-2">
                                            <button onClick={() => openEditor(ev)} className="text-xs text-neutral-400 hover:text-neutral-900 flex items-center gap-0.5">
                                                <Edit2 className="h-2.5 w-2.5" /> 수정
                                            </button>
                                            <button onClick={() => handleDelete(ev.id)} className="text-xs text-neutral-400 hover:text-red-500 flex items-center gap-0.5 ml-2">
                ...
                /* List view */
                <div className="border border-neutral-200 bg-white rounded">
                    <div className="grid grid-cols-[100px_1fr_100px_80px_80px_60px] px-4 py-2.5 text-xs font-medium text-neutral-400 uppercase border-b border-neutral-200 bg-neutral-50">
                        ...
                        upcomingEvents.map(ev => (
                            <div key={ev.id} className="grid grid-cols-[100px_1fr_100px_80px_80px_60px] px-4 py-2.5 border-b border-neutral-50 items-center hover:bg-neutral-50 transition-colors group">
                                ...
                                <div>
                                    ...
                                    {ev.description && <p className="text-xs text-neutral-400 truncate">{ev.description}</p>}
                                </div>
                                <span className="text-xs text-neutral-500">{ev.time}{ev.endTime ? `–${ev.endTime}` : ''}</span>
                                <span className={clsx("text-[11px] px-1.5 py-0.5 rounded font-medium w-fit", typeColors[ev.type])}>{ev.type}</span>
                                <span className="text-xs text-neutral-400 truncate">{ev.location || '-'}</span>
