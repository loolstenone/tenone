...
export default function ResumeUsersPage() {
  ...
  useEffect(() => {
    // 향후: hero.ne.kr 이력서 서비스 이용자 데이터 로드
    setLoading(false);
  }, []);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-neutral-400 mt-0.5">hero.ne.kr에서 이력서 서비스를 이용 중인 사람들</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link href="/hero/resume" target="_blank" className="flex items-center gap-1.5 text-xs text-neutral-400 hover:text-neutral-600">
      ...
      {loading ? (
        ...
        <div className="border border-neutral-200 rounded-lg p-12 text-center">
          ...
          <p className="text-sm text-neutral-400">아직 이력서 서비스 이용자가 없습니다</p>
          <p className="text-xs text-neutral-300 mt-1">hero.ne.kr에서 이력서 서비스가 시작되면 이용자가 표시됩니다</p>
        </div>
      ) : (
        <div className="border border-neutral-200 rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {users.filter(u => !search || u.name?.includes(search) || u.email?.includes(search)).map((u: any) => (
                <tr key={u.id} className="border-t border-neutral-100 hover:bg-neutral-50">
                  ...
                  <td className="px-4 py-3 text-xs text-neutral-400">{u.updatedAt}</td>
                </tr>
              ))}
            </tbody>
          </table>
