...
interface Member { id: string; name: string | null; email: string; created_at: string; }

export default function TownityMembersPage() {
    ...
    return (
        <div>
            <div className="mb-6">
                ...
                <p className="text-sm text-neutral-400 mt-0.5">Townity 가입 회원 관리</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="border border-neutral-200 rounded-lg overflow-hidden">
                    <table className="w-full text-sm">
                        <thead><tr className="bg-neutral-50 text-left">{["이름","이메일","가입일"].map(h => <th key={h} className="px-4 py-3 font-semibold text-neutral-500">{h}</th>)}</tr></thead>
                        <tbody>
                            {filtered.map(m => (
