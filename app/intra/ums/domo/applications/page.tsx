...
interface Application { id: string; name: string; email: string; company: string | null; message: string | null; status: string; created_at: string; }

const STATUS_MAP: Record<string, { l: string; c: string }> = {
    pending: { l: "대기", c: "bg-amber-50 text-amber-700" },
    approved: { l: "승인", c: "bg-emerald-50 text-emerald-700" },
    rejected: { l: "거절", c: "bg-red-50 text-red-600" },
};

export default function DomoApplicationsPage() {
    const [loading, setLoading] = useState(true);
    const [apps, setApps] = useState<Application[]>([]);
    const [filter, setFilter] = useState<string>("pending");

    useEffect(() => {
        createClient().from("domo_applications")
    ...
    const handleAction = async (id: string, newStatus: string) => {
        ...
        await sb.from("domo_applications").update({ status: newStatus }).eq("id", id);
    ...
    return (
        <div>
            <div className="mb-6">
                ...
                <p className="text-sm text-neutral-400 mt-0.5">Domo 가입 신청 심사 · 승인 관리</p>
            </div>
            <div className="grid grid-cols-3 gap-4 mb-6">
                {[{ l: "대기", k: "pending", c: "text-amber-600" }, { l: "승인", k: "approved", c: "text-emerald-600" }, { l: "거절", k: "rejected", c: "text-red-600" }].map(({ l, k, c }) => (
                    <div key={k} className="border border-neutral-200 rounded-lg p-4">
                        <p className="text-xs text-neutral-400 mb-1">{l}</p>
                        <p className={`text-2xl font-bold ${c}`}>{apps.filter(a => a.status === k).length}</p>
                    </div>
                ))}
            </div>
            <div className="flex gap-2 mb-4">
                {[{ l: "대기", v: "pending" }, { l: "승인", v: "approved" }, { l: "거절", v: "rejected" }, { l: "전체", v: "all" }].map(({ l, v }) => (
                            ...
                            <div key={a.id} className="border border-neutral-200 rounded-lg p-4">
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-neutral-500 mb-1">{a.email}{a.company && ` · ${a.company}`}</p>
                                        {a.message && <p className="text-xs text-neutral-600 bg-neutral-50 p-2 rounded mt-2">{a.message}</p>}
                                    </div>
                                    {a.status === "pending" && (
