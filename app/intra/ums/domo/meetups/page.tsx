...
export default function DomoMeetupsPage() {
    ...
    const filtered = meetups.filter(m => !search || m.title?.includes(search) || m.host_name?.includes(search));
        ...
        <div>
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm text-neutral-400 mt-0.5">Domo 비즈니스 네트워킹 모임 관리</p>
                </div>
                <div className="relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-neutral-300" />
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="모임명, 주최자..."
