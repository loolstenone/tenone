...
export async function GET(req: Request) {
    ...
    const [daily, weekly, monthly, yearly, projects, notes, briefings, identity] = await Promise.all([
        ...
        admin.from('myverse_projects')
            ...
            .ilike('title', ilike)
            .limit(10),
        admin.from('myverse_project_notes')
            .select('id, title, content, project_id, myverse_projects!inner(member_id)')
            .eq('myverse_projects.member_id', memberId)
            .or(`title.ilike.${ilike},content.ilike.${ilike}`)
            .limit(10),
        admin.from('myverse_ai_briefings')
            .select('id, briefing_type, briefing_date, content')
            .eq('member_id', memberId)
            .ilike('content', ilike)
            .order('briefing_date', { ascending: false })
            .limit(10),
        admin.from('myverse_identities')
