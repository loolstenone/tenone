...
export async function GET(_req: Request, { params }: { params: Promise<{ token: string }> }) {
    ...
    const [{ data: notes }, { data: milestones }, { data: vrief }, { data: gpr }, { data: owner }] = await Promise.all([
        ...
        admin.from("myverse_project_vriefs").select("*").eq("project_id", project.id).maybeSingle(),
        admin.from("myverse_project_gprs").select("*").eq("project_id", project.id).maybeSingle(),
