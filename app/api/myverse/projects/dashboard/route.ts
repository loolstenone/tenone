...
export async function GET(req: Request) {
    ...
    const { data: projects } = await admin
        ...
        .select("id, title, color, cover_id, category, end_date, status, myverse_project_gprs(progress)")
    ...
    type ProjectRow = {
        ...
        status: string;
        myverse_project_gprs: GprRow[] | GprRow | null;
    };

    const items = ((projects ?? []) as ProjectRow[]).map((p) => {
        const gprs = Array.isArray(p.myverse_project_gprs)
            ? p.myverse_project_gprs
            : p.myverse_project_gprs ? [p.myverse_project_gprs] : [];
