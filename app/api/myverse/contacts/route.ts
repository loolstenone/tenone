...
export async function POST(req: Request) {
    ...
    if (Array.isArray(body.contacts)) {
        ...
        if (rowsToInsert.length > 0) {
            const { data, error } = await admin.from("myverse_contacts").insert(rowsToInsert).select("id");
            if (error) return NextResponse.json({ error: error.message }, { status: 500 });
        ...
        if (rowsToUpdate.length > 0) {
            ...
            for (let i = 0; i < rowsToUpdate.length; i += CHUNK) {
                ...
                const results = await Promise.all(
                    slice.map(({ id, patch }) =>
                        admin.from("myverse_contacts").update(patch).eq("id", id).eq("member_id", memberId)
                    )
                );
...
export async function DELETE(req: Request) {
    ...
    if (id) {
        const { error } = await admin.from("myverse_contacts").delete().eq("id", id).eq("member_id", memberId);
        if (error) return NextResponse.json({ error: error.message }, { status: 500 });
