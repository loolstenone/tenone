...
// 라벨 관리 API
//   POST   /api/myverse/contacts/labels  — 일괄 적용/제거 { ids, add?, remove? }
//   PUT    /api/myverse/contacts/labels  — 라벨 이름 변경 { from, to }
//   DELETE /api/myverse/contacts/labels?name=xxx — 모든 연락처에서 라벨 제거
...
export async function POST(req: Request) {
    ...
    for (let i = 0; i < tasks.length; i += CHUNK) {
        ...
        const results = await Promise.all(
            slice.map(({ id, arr }) =>
                admin.from("myverse_contacts").update({ labels: arr, updated_at: new Date().toISOString() }).eq("id", id).eq("member_id", memberId)
            )
        );
...
export async function PUT(req: Request) {
    ...
    for (let from2 = 0; ; from2 += PAGE) {
        ...
        const results = await Promise.all(
            tasks.map(({ id, next }) =>
                admin.from("myverse_contacts").update({ labels: next, updated_at: new Date().toISOString() }).eq("id", id).eq("member_id", memberId)
            )
        );
        updated += results.filter(r => !r.error).length;
