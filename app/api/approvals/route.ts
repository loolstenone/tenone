...
export async function GET() {
    ...
    const { data, error, count } = await supabase!.from('wio_approvals')
...
export async function POST(request: NextRequest) {
    ...
    if (!body.doc_no) {
        ...
        const { count } = await supabase!.from('wio_approvals').select('*', { count: 'exact', head: true });
        body.doc_no = `APR-${year}-${String((count || 0) + 1).padStart(4, '0')}`;
    }

    const { data, error } = await supabase!.from('wio_approvals').insert(body).select().single();
