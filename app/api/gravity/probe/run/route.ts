 ...
 * ai-prober Edge Function을 트리거해 5대 AI 질의 실행
 *
 * Body:
 * {
 *   product_id: string,       // 필수
 *   brand_name: string,       // 필수 — 클라이언트 브랜드명
 *   competitors?: string[],   // 경쟁사 목록
 *   models?: string[],        // ['claude','chatgpt','gemini','perplexity']
 *   pattern_limit?: number,   // 질의할 패턴 수 (기본 10)
 * }
 */
export async function POST(req: NextRequest) {
    const body = await req.json().catch(() => ({}));
    const { product_id, brand_name, competitors, models, pattern_limit } = body;
    ...
    const res = await fetch(`${supabaseUrl}/functions/v1/ai-prober`, {
        method: "POST",
        headers: {
    ...
    if (!res.ok) {
        console.error("[gravity/probe/run] edge function error:", data);
