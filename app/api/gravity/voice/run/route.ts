 ...
 * voice-designer Edge Function 트리거
 *
 * Body:
 * {
 *   product_id: string,
...
export async function POST(req: NextRequest) {
    ...
    const { product_id, brand_name, top_n } = body;
    ...
    const res = await fetch(`${supabaseUrl}/functions/v1/voice-designer`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${serviceRoleKey}`,
        },
        body: JSON.stringify({ product_id, brand_name, top_n }),
    });

    const data = await res.json().catch(() => ({}));

    if (!res.ok) {
        console.error("[gravity/voice/run] edge function error:", data);
