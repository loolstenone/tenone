...
export async function POST(req: Request) {
    ...
    if (projects.data && projects.data.length > 0) {
        ctx.push(`[진행 중 프로젝트]`);
        projects.data.forEach((p: { title: string; planners_project_gprs: Array<{ progress: number }> | { progress: number } | null }) => {
            const prog = Array.isArray(p.planners_project_gprs)
