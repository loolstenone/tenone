 ...
 * 방 카테고리/메타데이터 기반으로 페르소나를 선택하고 badaksoe 에이전트를 실행.
 *
  ...
  try {
    ...
    if (room_id) {
      ...
      if (room) {
        ...
        // metadata.persona_override가 있으면 우선 사용
    ...
    // badaksoe 에이전트 실행
    // system_prompt는 DB에서 가져오고, 방 페르소나를 앞에 주입
    ...
    console.error('[Badaksoe] 오류:', error);
    return NextResponse.json(
