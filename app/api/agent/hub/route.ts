...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 에이전트 이름 결정
    let targetAgent = agentName || '1001';

    // 1001 라우팅: agentName 미지정 시 1001에게 먼저 물어본다
    if (!agentName) {
      ...
      if (orchestratorProfile) {
        ...
        const routingPrompt =
          ...
          `사용 가능한 에이전트: ${orchestratorProfile.can_invoke.join(', ')}\n` +
          `만약 특정 에이전트가 명확하지 않으면 "1001"을 반환하세요.\n\n` +
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            if (parsed.agent && parsed.agent !== '1001') {
              // 라우팅된 에이전트가 존재하는지 확인
