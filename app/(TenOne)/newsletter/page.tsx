...
export default function NewsletterPage() {
    ...
    return (
        <div className="max-w-4xl mx-auto px-6 py-16">
            ...
            {/* 구독 폼 */}
            <div className="bg-neutral-50 border border-neutral-200 p-8 mb-16">
                {subscribed || isMemberSubscribed ? (
                    <div className="text-center py-4">
                        ...
                        {isAuthenticated && (
                            <Link href="/profile" className="inline-block mt-3 text-xs text-neutral-400 hover:text-neutral-700 underline underline-offset-2">
                                프로필에서 구독 관리 →
                            </Link>
                        )}
                    </div>
                    ...
                    <div className="text-center py-2">
                        ...
                        <p className="text-[10px] text-neutral-400 mt-3">격주 발행 · 언제든 프로필에서 구독 취소 가능</p>
                    </div>
                ) : (
                    /* 비회원 구독 폼 */
                    <form onSubmit={handleSubscribe}>
                        <div className="flex flex-col sm:flex-row items-center gap-3 mb-3">
                            <Mail className="h-5 w-5 text-neutral-400 shrink-0 hidden sm:block" />
                            <input
                                type="text" value={guestName} onChange={e => setGuestName(e.target.value)}
                                placeholder="이름 (선택)"
                                className="w-full sm:w-40 px-4 py-3 text-sm border border-neutral-200 bg-white focus:outline-none focus:border-neutral-400 placeholder:text-neutral-300"
                            />
                            <input
                                type="email" value={email} onChange={e => setEmail(e.target.value)}
                                placeholder="이메일 주소를 입력하세요 *"
                                required
                                className="flex-1 w-full px-4 py-3 text-sm border border-neutral-200 bg-white focus:outline-none focus:border-neutral-400 placeholder:text-neutral-300"
                            />
                        ...
                        <div className="flex items-start gap-2 mt-2">
                            <button type="button" onClick={() => setAgreePrivacy(!agreePrivacy)}
                                className={`mt-0.5 w-4 h-4 border rounded flex items-center justify-center shrink-0 transition-colors ${
                                    agreePrivacy ? 'bg-neutral-900 border-neutral-900' : 'border-neutral-300 bg-white'
                                }`}>
                        ...
                        <p className="text-[10px] text-neutral-300 text-center mt-3">격주 발행 · 언제든 구독 취소 가능 · 회원 가입 없이 구독 가능</p>
                    </form>
                )}
            </div>
            ...
            <div>
                ...
                <div className="space-y-3">
                    {pastIssues.map(issue => (
                        <div key={issue.id} className="border border-neutral-200 bg-white p-5 hover:border-neutral-400 transition-colors cursor-pointer group">
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${categoryStyle[issue.category] || "bg-neutral-100 text-neutral-500"}`}>
                                            {issue.category}
                                        </span>
                                ...
                                <ExternalLink className="h-4 w-4 text-neutral-300 group-hover:text-neutral-500 transition-colors shrink-0 mt-1" />
                            </div>
                        </div>
                    ))}
                </div>
