...
export default function MarketUploadPage() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        if (!creator || !category || !title.trim() || !price) return;
        ...
        if (!product) { setError("상품 등록에 실패했습니다. 다시 시도해주세요."); setSubmitting(false); return; }
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            <div className="border-b border-neutral-200 px-5 py-3 flex items-center justify-between">
                ...
                <p className="text-[12px] font-bold text-neutral-900">상품 등록</p>
                <div className="w-14" />
            </div>
            ...
            <form onSubmit={handleSubmit} className="max-w-xl mx-auto px-5 py-8 space-y-7">
                ...
                >
                    {submitting ? (
                        ...
                        <><Package className="w-4 h-4" /> 마켓에 등록하기</>
                    )}
                </button>
            </form>
        </div>
