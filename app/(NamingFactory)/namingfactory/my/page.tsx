...
export default function NamingFactoryMyPage() {
    ...
    useEffect(() => {
        if (!user?.id) return;
        fetch(`/api/board/posts?site=namingfactory&limit=20&status=published`).then(r => r.json()).then(d => setMyPosts(d.posts || [])).catch(() => {});
    }, [user?.id]);

    if (isLoading) return (
        <div className="min-h-screen flex items-center justify-center bg-neutral-950">
            <div className="h-6 w-6 border-2 border-[#FCD34D]/20 border-t-[#FCD34D] rounded-full animate-spin" />
        </div>
    );
    if (!isAuthenticated) return (
        <div className="min-h-screen bg-neutral-950">
            <LoginModal isOpen={true} onClose={() => {}} accentColor="#FCD34D" />
        </div>
    );
    ...
    return (
        <div className="min-h-screen pt-24 pb-20 px-6 bg-neutral-950 text-neutral-100">
            <div className="max-w-4xl mx-auto">
                <MyProfileCard accentColor="#FCD34D" siteBadge="네이밍팩토리" />

                {user?.id && <CapabilitySection memberId={user.id} brandId="namingfactory" accentColor="#FCD34D" className="mb-6" />}

                <div className="flex items-center gap-1 mb-8 border-b border-neutral-800">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={`flex items-center gap-2 px-4 py-3 text-sm border-b-2 transition-colors ${activeTab === tab.id ? "border-[#FCD34D] text-[#FCD34D]" : "border-transparent text-neutral-500 hover:text-neutral-300"}`}>
