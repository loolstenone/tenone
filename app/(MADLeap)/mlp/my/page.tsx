...
export default function MADLeapMyPage() {
    ...
    if (isLoading || !isAuthenticated) return <div className="min-h-screen flex items-center justify-center bg-neutral-900"><div className="h-6 w-6 border-2 border-neutral-600 border-t-[#7C3AED] rounded-full animate-spin" /></div>;
        ...
        <div className="min-h-screen pt-24 pb-20 px-6 bg-neutral-900 text-neutral-100">
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center gap-6 mb-10">
                    <div className="w-20 h-20 rounded-full bg-[#7C3AED]/20 flex items-center justify-center text-2xl font-bold text-[#7C3AED]">{initials}</div>
                    <div>
                        <h1 className="text-2xl font-bold">{user?.name || "회원"}</h1>
                        <p className="text-sm text-neutral-400 mt-1">{user?.email}</p>
                        <p className="text-xs text-[#7C3AED] mt-1">MADLeap Leaper</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-1 mb-8 border-b border-neutral-700">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={`flex items-center gap-2 px-4 py-3 text-sm border-b-2 transition-colors ${activeTab === tab.id ? "border-[#7C3AED] text-[#7C3AED]" : "border-transparent text-neutral-500"}`}>
