      ...
      <main className="flex-1">
          ...
          <div className="mx-auto max-w-3xl text-center">
            ...
            <div className="mx-auto max-w-lg">
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <button onClick={handleScan} className="flex items-center justify-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-semibold text-[#0A0E1A] transition-all hover:bg-white/90">
                  무료 진단 시작
                </button>
              </div>
              <p className="mt-3 text-xs text-white/30">회원가입 없이 · 30초 소요 · 완전 무료</p>
            </div>
          </div>
        </section>
        ...
        <section className="border-t border-border bg-surface px-5 py-16">
          <div className="mx-auto max-w-4xl">
            <p className="mb-8 text-center text-2xl font-bold text-text">GEO + SEO 통합 점검</p>
              ...
              <div className="rounded-2xl border border-border bg-white p-6">
                ...
                <div className="space-y-2.5 text-sm text-text-sub">
                  {['ChatGPT에서 브랜드 노출 여부', 'Perplexity 출처 인용 확인', 'Google AI Overview 포함 여부', '네이버 AI (Cue) 답변 포함', 'AI 최적화 준비도'].map((item, i) => (
                    <div key={i} className="flex items-center gap-2"><div className="h-1 w-1 rounded-full bg-text" />{item}</div>
                  ))}
                </div>
              </div>
              <div className="rounded-2xl border border-border bg-white p-6">
        ...
        <section id="process" className="px-5 py-16">
          <div className="mx-auto max-w-3xl">
            <p className="mb-10 text-center text-2xl font-bold text-text">진단 → 기획 → 제작 → 실행 → 분석</p>
            <div className="space-y-4">
              {PROCESS_STEPS.map((step) => (
                <div key={step.num} className="flex items-start gap-5 rounded-2xl border border-border bg-white p-5 transition-colors hover:bg-surface">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-surface text-text">{step.icon}</div>
        ...
        <section className="border-t border-border bg-surface px-5 py-16">
          <div className="mx-auto max-w-4xl">
            <p className="mb-3 text-center text-2xl font-bold text-text">URL 하나로 이 모든 분석을</p>
            <p className="mb-10 text-center text-sm text-text-sub">무료 진단으로 시작하고, 회원가입하면 심층 리포트를 받아보세요</p>
        ...
        <section className="px-5 py-20">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="mb-4 text-2xl font-bold text-text">30초면 알 수 있습니다</h2>
            <p className="mb-8 text-sm text-text-sub">AI가 추천하지 않는 사이트는 이미 고객을 잃고 있을 수 있습니다</p>
