...
| **이월 작업 (2026-05-20 갱신)** | **외부 키 발급 선행 필요 (블로커):** ① ANTHROPIC_API_KEY 401 진단 — **자동 만료 아님**, `.env.local`(집·사무실) ↔ Vercel Env 3곳 키 불일치 / revoke·rotate / 결제·크레딧 / workspace 중 진단 → 새 키 1회 셋업으로 종결 (Claude probe·exec summary 잠금) · ② OpenAI/Perplexity/SerpAPI/PageSpeed 키 → 5 AI 플랫폼 + CWV 전체 활성 · ③ Ahrefs/Moz API → `persistence_score` N/A 해소 · ④ Google Ads/Meta/Naver Ads/Kakao Biz API → 캠페인 spent·CTR 자동 갱신 (Phase C) · ⑤ FCM/Kakao Bizmessage → 실 발송 활성 · ⑥ Resend webhook → email_events 자동 캡처. **내부 작업:** ⑦ `wio_feature_flags` 연동 · ⑧ localStorage `smarcomm_favorites` → `user_settings` 통합 · ⑨ Phase 5 운영 자동화 강화 · ⑩ AIRM 4단계 검증(④) UI 보강. **USER_SCENARIO.md § 5 갭** 17건 별도 추적. |
...
| **블로커** | Toss 가맹점 승인 + Vercel 환경변수 (실 결제 흐름 작동 안 됨) · ANTHROPIC_API_KEY 401 (자동 만료 아님 — 진단 후 1회 셋업으로 종결) · Marvis #1 대시보드·#3 RFM 분석·#4 이메일 1탭 = 카페24 dev sandbox 등록 + webhook 미연결 (사용자 직접 작업) |
| **Marvis 진척 (세션 144)** | **#2 진단 활성화 완료** — `/smarcomm/marvis/scan` `disabled` 해제 + URL 검증 + `/smarcomm/scan?url=...&from=marvis`로 기존 SmarComm Index 엔진(`/api/smarcomm/scan`) 위임. ANTHROPIC_API_KEY 없어도 기술 SEO·Schema·CWV는 측정, Citability만 N/A. 결과는 기존 `/smarcomm/report/[id]`. **#1·#3·#4 골격은 다음 세션** (카페24 등록 후). |
| **세션 150 (2026-05-25)** | **D.Frame 벤치마킹 + Phase 3.5·3.4·3.2 일괄 완료** — ① **Phase 3.5 정량 콘텐츠** (commit `93016ce2`): 랜딩 hero 아래 "How We Score" 3카드 (산식 투명·실측 누적·정직성). [HowWeScoreSection](features/smarcomm/HowWeScoreSection.tsx) + [API benchmark-stats](../../app/api/smarcomm/benchmark-stats/route.ts) ISR 10분. 누적 19건 실측 평균 노출. ② **Phase 3.4 환각 감지** 🥇 D.SaiO 대응 (commit `37d780e0`): 기존 `classifySentimentLLM` factComparisons 활용 — 신규 LLM 모듈 0. [smarcomm_brand_facts·smarcomm_hallucinations](../../sql/smarcomm-hallucinations.sql) 테이블 + [hallucination-persist.ts](../../lib/smarcomm/hallucination-persist.ts) + [HallucinationCard.tsx](features/smarcomm/HallucinationCard.tsx). 백필 검증: hsad.co.kr scan에서 "AI 1974년 vs 사이트 1984년" 자동 감지. Report에 3단계 분류 (factual_error/partial_match/unverifiable) 카드 노출. ③ **Phase 3.2 Web Push 마무리** (commit `1f2eb655`): VAPID 키 발급 (.env.local) + send API 권한 게이트 (member_roles staff/manager/super_admin) + [PushSubscribeButton.tsx](features/smarcomm/PushSubscribeButton.tsx) 4상태 UI + [offline page](smarcomm/offline/page.tsx). 이전 세션 미커밋 잔재(push.ts·sw·subscribe API·DB) 흡수. **블로커**: Vercel Env 등록은 사용자, e2e 검증 대기. **Phase 4 (Marvis Phase 1)**: OAuth(카페24/Imweb) 없이는 RFM·매출 데이터 0건 → 정직성 위반 위험 → 보류 결정. |
| **세션 149 끝물 정리 (2026-05-23)** | 랜딩 hero 아래 Marvis 4카드 섹션 삭제 (commit `ce4c99ff`, [page.tsx](smarcomm/page.tsx)). "1탭·카페24·매일·Phase 1" 미완료 라벨 노출은 § 1.10 정직성 ZERO 위반 가능성 — hero → 바로 "GEO + SEO 통합 점검" 섹션으로 연결. |
| **세션 149 (2026-05-23)** | **Phase 3.1.2 완료 — 캠페인 작성 모달 UI (분리 컴포넌트화)**. ① [features/smarcomm/EmailCampaignModal.tsx](features/smarcomm/EmailCampaignModal.tsx) 352라인 신규 — 캠페인 이름·제목·본문·프리헤더·버튼 라벨/URL·발신자 select + **세그먼트 선택** (`/api/smarcomm/crm/segments` 연동) + **본문 작성/미리보기 탭** + **테스트 발송 UI** + `person_ids` 직접 입력. ② [crm/email/page.tsx](smarcomm/dashboard/crm/email/page.tsx) — "+ 새 캠페인" 버튼 (헤더·빈 상태) + `<EmailCampaignModal />` 호출, 인라인 form/state/handlers 전부 제거. ③ 랜딩 hero 회전 카피·분석 경로 복구 (별도, [페이지](smarcomm/page.tsx)) — `HEADLINES` 20개 회전 + `/smarcomm/scan?url=...` 풀 SmarComm Index 진입 복귀. **이월 해소**: `body_html` 에디터·세그먼트·테스트 발송 UI 셋 다 단일 컴포넌트로 통합. **다음**: Phase 3.2 (웹 푸시 VAPID·서비스 워커·`smarcomm_push_subscriptions`) + EmailCampaignModal 회귀 검증 (실 segments 응답·테스트 발송 e2e). |
| **세션 147 (2026-05-21)** | **Phase 3.1 옵션 A 완료 — 이메일 발송 헬퍼 분리 + SmarComm 라우트 신설**. ① [lib/email/send-broadcast.ts](../../lib/email/send-broadcast.ts) 신설 — 인트라 `/api/intra/crm/broadcast/send/route.ts` 71~228줄을 `sendCampaignBroadcast()`로 추출, `BroadcastError(message, status)` 헬퍼 에러 클래스. ② 인트라 라우트 229→43줄 리팩, 헬퍼 호출로 변경. ③ `crm_campaigns`에 `created_by_service` + `owner_user_id` 컬럼 추가 + "crm_campaigns smarcomm owner" RLS 정책 ([sql/crm-campaigns-owner-columns.sql](../../sql/crm-campaigns-owner-columns.sql), prod 적용 완료). ④ [app/api/smarcomm/email/send/route.ts](../../app/api/smarcomm/email/send/route.ts) 신설 — 본인 캠페인 한정 + 헬퍼 호출. ⑤ [app/api/smarcomm/email/campaigns/route.ts](../../app/api/smarcomm/email/campaigns/route.ts) GET·POST + [crm/email/page.tsx](smarcomm/dashboard/crm/email/page.tsx) "내 캠페인" 섹션 + "지금 발송" 버튼. **검증**: 401/200 정상, 서버·콘솔 에러 0. **이월**: Phase 3.1.2 (캠페인 작성 모달 UI) — `body_html` 에디터·세그먼트 선택·테스트 발송 UI는 신규. **모순 식별 재확인**: `smarcomm_broadcasts` 미사용 — 옵션 C로 가는 길 1단계 완료. |
| **세션 145 (2026-05-21)** | **Workspace 정합성 + TierGate SSOT + Phase 3 설계**. ① 사이드바 32개 vs 파일 시스템 교차 감사 — 404 0건. orphan stub 6개 + dead workflow-context 사본 삭제. ② [lib/smarcomm/tier-policy.ts](../../lib/smarcomm/tier-policy.ts) **신설 SSOT** — TierGate·DashboardSidebar·layout 3자 통합. layout이 path 기반으로 children 자동 wrap → 페이지 20개 손 안 대고 티어 게이트 적용. ③ [docs/SmarComm_Phase3_Plan.md](../../docs/SmarComm_Phase3_Plan.md) 신설 — 카카오·푸시·이메일·자동화 인프라 + §9에 옵션 A/B/C 비교 (권장 A: 인트라 발송 인프라 헬퍼 추출 + SmarComm 라우트). ④ Marvis 라이트 페이지 제거, 풀 버전으로 통합. **모순 식별**: `smarcomm_broadcasts` vs `crm_campaigns` 2개 시스템 분리 — Phase 3.1에서 옵션 A로 단계 통합. |
