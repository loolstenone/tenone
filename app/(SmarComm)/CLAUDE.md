...
| **개발 수준** | 진단 영역 = 산업 표준급 보고서 완성 (Phase 1~3). 나머지 35+ 페이지는 mock. API 라우트 6개 (scan + report/[id] + report/[id]/trend + creative + advisor + **cron/smarcomm-weekly-rescan**). **Phase 5 Item 1 완료** — `lib/smarcomm/run-scan.ts` 공유 파이프라인 + `smarcomm_rescan_schedules` 테이블 Prod 적용. Smart-Data Hub · AIRM · 자산화 모듈 미구현 (SSOT 잠금). |
