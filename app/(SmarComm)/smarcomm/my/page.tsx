...
export default function SmarCommMyPage() {
    ...
    useEffect(() => {
        if (!user?.id) return;
        fetch(`/api/board/posts?site=smarcomm&limit=20&status=published`).then(r => r.json()).then(d => setMyPosts(d.posts || [])).catch(() => {});
    }, [user?.id]);

    if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-neutral-900"><div className="h-6 w-6 border-2 border-neutral-600 border-t-[#8B5CF6] rounded-full animate-spin" /></div>;
    if (!isAuthenticated) return <div className="min-h-screen bg-neutral-900"><LoginModal isOpen={true} onClose={() => {}} accentColor="#8B5CF6" /></div>;
    ...
    return (
        <div className="min-h-screen pt-24 pb-20 px-6 bg-neutral-900 text-neutral-100">
            <div className="max-w-4xl mx-auto">
                ...
                <MyProfileCard accentColor="#8B5CF6" />

                {user?.id && <CapabilitySection memberId={user.id} brandId="smarcomm" accentColor="#8B5CF6" className="mb-6" />}

                <div className="flex items-center gap-1 mb-8 border-b border-neutral-700">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={`flex items-center gap-2 px-4 py-3 text-sm border-b-2 transition-colors ${activeTab === tab.id ? "border-[#8B5CF6] text-[#8B5CF6]" : "border-transparent text-neutral-500"}`}>
