...
export default function MullaesianMyPage() {
    ...
    useEffect(() => {
        if (!user?.id) return;
        fetch(`/api/board/posts?site=mullaesian&limit=20&status=published`).then(r => r.json()).then(d => setMyPosts(d.posts || [])).catch(() => {});
    }, [user?.id]);

    if (isLoading) return (
        <div className="min-h-screen flex items-center justify-center bg-neutral-950">
            <div className="h-6 w-6 border-2 border-[#29ABE2]/20 border-t-[#29ABE2] rounded-full animate-spin" />
        </div>
    );
    if (!isAuthenticated) return (
        <div className="min-h-screen bg-neutral-950">
            <LoginModal isOpen={true} onClose={() => {}} accentColor="#29ABE2" />
        </div>
    );
    ...
    return (
        <div className="min-h-screen pt-24 pb-20 px-6 bg-neutral-950 text-neutral-100">
            <div className="max-w-4xl mx-auto">
                ...
                <MyProfileCard accentColor="#29ABE2" siteBadge="뮬라지안" />

                {user?.id && <CapabilitySection memberId={user.id} brandId="mullaesian" accentColor="#29ABE2" className="mb-6" />}

                <div className="flex items-center gap-1 mb-8 border-b border-neutral-800">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={`flex items-center gap-2 px-4 py-3 text-sm border-b-2 transition-colors ${activeTab === tab.id ? "border-[#29ABE2] text-[#29ABE2]" : "border-transparent text-neutral-500 hover:text-neutral-300"}`}>
