...
export default function HeRoMyPage() {
    ...
    if (isLoading || !isAuthenticated) return <div className="min-h-screen flex items-center justify-center bg-neutral-900"><div className="h-6 w-6 border-2 border-neutral-600 border-t-[#0EA5E9] rounded-full animate-spin" /></div>;
        ...
        <div className="min-h-screen pt-24 pb-20 px-6 bg-neutral-900 text-neutral-100">
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center gap-6 mb-10">
                    <div className="w-20 h-20 rounded-full bg-[#0EA5E9]/20 flex items-center justify-center text-2xl font-bold text-[#0EA5E9]">{initials}</div>
                    <div>
                        <h1 className="text-2xl font-bold">{user?.name || "회원"}</h1>
                        <p className="text-sm text-neutral-400 mt-1">{user?.email}</p>
                        <p className="text-xs text-[#0EA5E9] mt-1">HeRo Member</p>
                    </div>
                </div>
