...
const MODULE_ORDER: DModuleType[] = ['expertise', 'leadership', 'identity', 'network', 'seniorReadiness'];
...
function HitDTestContent() {
  ...
  const currentModule = currentQuestion?.module ?? 'expertise';
  ...
  useEffect(() => {
    async function init() {
      ...
      const grouped = new Map<DModuleType, QuestionItem[]>(MODULE_ORDER.map((m) => [m, []]));
      for (const row of rows) {
        const mod = row.module as DModuleType;
      ...
      try {
        const res = await fetch(`/api/hit/d/session/${sessionToken}`);
        if (res.ok) {
  ...
  const submitResults = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/hit/d/score', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sessionToken,
      ...
      if (res.ok) {
        const data = await res.json();
        setIsComplete(true);
        router.push(`/hero/hit/d/result/${data.resultId}`);
      } else {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <p className="text-neutral-400">
          세션을 찾을 수 없습니다.{" "}
          <a href="/hero/hit/d" className="text-[#E53935] underline">
            다시 시작
          </a>
          해 주세요.
        </p>
  ...
  if (isSubmitting || isComplete) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <p className="text-neutral-500 text-sm">시니어 리더십 전환 리포트를 생성하고 있습니다...</p>
