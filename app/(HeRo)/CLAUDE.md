...
- **유형**: 오픈 → 사업 시작 후 구매/구독 전환 예정
- **현재 운영 방식**: 무료 검사 + 씨치 라이트(TIH) 기업 인재 의뢰
- **사업 개시 후 결제 모델** (PG 연동은 사업 시작 시점에 구현):
  1. 회원가입 (이메일)
  2. 무료 검사 또는 유료 검사 선택
  3. 결제 (Stripe/Toss) — **미구현, 사업 시작 시 연동**
  4. HIT 검사 실시 (총 60분)
  5. 결과 도출 → 커리어 코칭 시작
- **멤버 권한**:
  - `member` — 기본 회원 (무료 콘텐츠)
  - `purchaser` — 검사 구매자 (결과 조회) — 사업 시작 후 활성화
  - `subscriber` — 구독 상담자 (정기 코칭) — 사업 시작 후 활성화
...
| **이월 작업** | TIH 제출 500 에러 실기기 최종 확인(fix 배포됨, 테스트 미완) |
| **보류 (사업 시작 시)** | `/hero/coaching/ai` 결제 PG 연동 (Stripe/Toss) — 사이트 완료 후 사업 개시 시점에 구현 |
