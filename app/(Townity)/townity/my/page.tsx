...
export default function TownityMyPage() {
    ...
    useEffect(() => {
        if (!user?.id) return;
        fetch(`/api/board/posts?site=townity&limit=20&status=published`).then(r => r.json()).then(d => setMyPosts(d.posts || [])).catch(() => {});
    }, [user?.id]);

    if (isLoading) return (
        <div className="min-h-screen flex items-center justify-center bg-neutral-950">
            <div className="h-6 w-6 border-2 border-[#10B981]/20 border-t-[#10B981] rounded-full animate-spin" />
        </div>
    );
                ...
                <div className="flex items-center gap-1 mb-8 border-b border-neutral-800">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={`flex items-center gap-2 px-4 py-3 text-sm border-b-2 transition-colors ${activeTab === tab.id ? "border-[#10B981] text-[#10B981]" : "border-transparent text-neutral-500 hover:text-neutral-300"}`}>
