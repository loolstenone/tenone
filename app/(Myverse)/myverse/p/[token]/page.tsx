...
export async function generateMetadata(
    ...
    const ownerName = owner?.name ?? "마이버스";
    ...
    const description = project.retrospective?.fact?.trim()
        ...
        : `${ownerName}이(가) 마이버스 AI로 기록한 프로젝트입니다.`;
    ...
    return (
        <div className="min-h-screen bg-neutral-50">
            <div className="max-w-3xl mx-auto px-6 py-12">
                <Link
                    href="/myverse"
                    className="inline-flex items-center gap-1 text-xs text-neutral-500 hover:text-[#6366F1] mb-6"
                >
                ...
                <header className="mb-8 pb-6 border-b border-neutral-200">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        {project.status === "completed" && (
                            <span className="inline-flex items-center gap-1 text-[11px] text-[#6366F1]">
                                ...
                                {milestones.map((m) => (
                                    <li key={m.id} className="flex items-center gap-2">
                                        <span className={`w-3 h-3 rounded border-2 flex items-center justify-center shrink-0 ${
                                            m.done_at ? "bg-[#6366F1] border-[#6366F1]" : "border-neutral-300"
                                        }`}>
