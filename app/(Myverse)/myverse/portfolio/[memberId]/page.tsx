...
export async function generateMetadata(
    ...
    const title = `${ownerName}의 프로젝트 포트폴리오 — 마이버스`;
    const description = `${ownerName}이(가) 마이버스 AI로 기록한 공개 프로젝트 ${count ?? 0}개`;
...
function StatusBadge({ status }: { status: string }) {
    const map: Record<string, { label: string; cls: string }> = {
        active:    { label: "진행중",  cls: "bg-indigo-50 text-indigo-700" },
    ...
    return (
        <div className="min-h-screen bg-neutral-50">
            <div className="max-w-4xl mx-auto px-6 py-12">
                <Link
                    href="/myverse"
                    className="inline-flex items-center gap-1 text-xs text-neutral-500 hover:text-[#6366F1] mb-8"
                >
                    <ChevronLeft className="h-3 w-3" /> 마이버스 AI
                </Link>
                ...
                <header className="mb-10 pb-8 border-b border-neutral-200">
                    <div className="flex items-center gap-4 mb-5">
                        {owner.avatar_url ? (
                            ...
                            <div className="w-14 h-14 rounded-full bg-[#6366F1] flex items-center justify-center text-white font-semibold text-xl">
                    ...
                    <div className="flex items-center gap-6 text-sm text-neutral-500">
                        ...
                        {activeCount > 0 && (
                            <span>
                                <strong className="text-indigo-700 font-semibold">{activeCount}</strong> 진행중
                            </span>
                        )}
                        {completedCount > 0 && (
                            <span>
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                    {projects.map((p) => {
                        ...
                        const progress = p.milestone_total > 0
                            ...
                            : null;
                        const accentColor = p.color || "#6366F1";

                        return (
                            <Link
                                key={p.id}
                                href={`/myverse/p/${p.public_token}`}
                                className="bg-white border border-neutral-200 rounded-xl p-5 hover:border-[#6366F1] hover:shadow-sm transition-all group block"
                            >
                                <div
                                ...
                                <div className="flex items-start justify-between gap-2 mb-3">
                                    <h2 className="font-semibold text-neutral-900 group-hover:text-[#6366F1] transition-colors leading-snug">
                                        {p.title}
                                    </h2>
