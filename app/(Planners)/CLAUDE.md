...
| 항목 | 내용 |
|------|------|
| **Phase** | **세션 99 (2026-04-29)** — **노트 병기(비파괴 모드 전환)** (Daily·Project 텍스트↔손글씨 전환 시 기존 내용 보존 — DailyView: `handwriting.text` 백업, ProjectNotesTab: `handModeOverride` + HW_MARKER `.text` 필드) · **노트 모달 이중 제목 삭제** (부제 hint 제거) · **캔버스 UI 통일** (Project 카드 neutral 색상, Daily 모달 아이콘 `text-sky-500`) · **AI Sparkles 제거** (DailyView import 삭제) |
