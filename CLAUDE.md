...
> ⚠️ **Vercel 비용 관리 (필수)**
> - **작업 중 push 금지** — push는 "작업 종료" 시 1회만. push할 때마다 Vercel 자동 배포가 트리거되어 크레딧이 소진된다.
> - **commit은 자유** — 로컬 commit은 비용 없음. push만 신중하게.
> - **로컬 확인 우선** — `npm run dev`로 확인. 로컬 서버는 Vercel 비용 무관.
> - **프리뷰 배포 차단됨** — `vercel.json`에서 dev/feature-* 브랜치 배포 비활성화 설정 완료.
> - **On-Demand 상한** — Spend Management $100 설정됨. 초과 과금 불가.
...
- ❌ 작업 중간에 push하기 (Vercel 배포 트리거 → 크레딧 소진. push는 작업 종료 시 1회만)
