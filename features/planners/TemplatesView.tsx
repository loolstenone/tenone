...
function EmotionLogGrid({ data, onChange }: { data: FrameworkData; onChange: (key: string, val: string) => void }) {
    ...
    return (
        <div className="my-2 space-y-2">
            ...
            <div className="rounded-xl p-3 bg-stone-50 border border-stone-200">
                <p className="text-[10px] font-bold text-stone-700 uppercase tracking-wider mb-2">Mood · 오늘의 감정 (복수 선택 가능)</p>
                    ...
                    {MOODS.map(m => {
                        ...
                        return (
                            <button key={m} onClick={() => {
                                ...
                                className={`px-3 py-1.5 rounded-full text-xs transition-all ${selected ? "bg-slate-900 text-white font-semibold" : "bg-white border border-stone-200 text-stone-700 hover:bg-stone-50"}`}>
                                {m}
                            </button>
                        );
                    })}
                    ...
                    <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map(n => (
                            <button key={n} onClick={() => onChange("emo_intensity", String(n))}
                                className={`flex-1 py-1.5 text-xs rounded ${intensity >= n ? "bg-slate-900 text-white font-bold" : "bg-white border border-stone-200 text-neutral-400"}`}>
                                {n}
                            </button>
                        ))}
                    </div>
            ...
            <LabeledBox label="Trigger · 계기 (사실만)" valKey="emo_trigger" data={data} onChange={onChange} color="bg-stone-50 border-stone-200" textColor="text-stone-800" placeholder="예: 회의에서 내 제안이 받아들여지지 않음 · 어머니 전화 못 받음" />
            <LabeledBox label="Body · 몸의 신호" valKey="emo_body" data={data} onChange={onChange} color="bg-slate-50 border-slate-300" textColor="text-slate-900" placeholder="예: 어깨가 무겁다 · 가슴 두근 · 속이 답답 · 손이 차가워짐" />
            <LabeledBox label="Thought · 자동적 사고" valKey="emo_thought" data={data} onChange={onChange} color="bg-slate-50 border-slate-200" textColor="text-slate-800" placeholder='예: "역시 나는 부족해" / "팀에서 나만 뒤처져" / "이러다 잘릴 것 같아"' />
            <LabeledBox label="Reframe · 다시 본다면" valKey="emo_reflection" data={data} onChange={onChange} color="bg-slate-50 border-slate-300" textColor="text-slate-900" placeholder={"예: 한 번의 거절이 나를 정의하진 않는다. 다음 미팅에서 보강해 다시 제안하면 된다."} />

            {/* Pattern over time */}
            <div className="rounded-lg p-3 bg-stone-50 border border-stone-200">
                <p className="text-xs font-bold text-stone-900">Pattern · 같은 감정 반복?</p>
    ...
    return (
        <div className="my-2 space-y-2">
            ...
            <div className="rounded-xl p-4 bg-stone-50 border border-stone-200">
                <p className="text-[10px] font-bold text-stone-800 uppercase tracking-wider text-center mb-3">오늘 감사한 일 세 가지 — 왜 그것이 좋았는가까지</p>
                        ...
                        <div key={s.n} className="flex items-start gap-2">
                            ...
                            <div className="flex-1">
                                ...
                                <textarea value={data[`grat_${s.n}`] ?? ""} onChange={e => onChange(`grat_${s.n}`, e.target.value)}
                                    ...
                                    rows={2}
                                    className="w-full resize-none bg-white/60 text-xs p-2 rounded border border-stone-200 focus:outline-none focus:border-slate-700 leading-relaxed" />
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <LabeledBox label="Highlight · 오늘의 하이라이트 (한 장면)" valKey="grat_highlight" data={data} onChange={onChange} color="bg-slate-50 border-slate-300" textColor="text-slate-900" placeholder="예: 저녁에 가족과 같이 새 메뉴 시도하며 웃음" />
                <LabeledBox label="Tomorrow · 내일 기대하는 것" valKey="grat_tomorrow" data={data} onChange={onChange} color="bg-slate-50 border-slate-200" textColor="text-slate-800" placeholder="예: 새 책 시작 / 산책 코스 신규 시도" />
            </div>

            {/* Letter */}
            <div className="rounded-xl p-3 bg-stone-50 border border-stone-300">
                <p className="text-xs font-bold text-stone-900">Gratitude Letter (선택) · 누군가에게 한 줄 감사</p>
        ...
        <div className="my-2 space-y-2">
            ...
            <div className="rounded-xl p-3 bg-stone-50 border border-stone-200 space-y-2">
                <div className="flex gap-3 items-start">
                    <div className="shrink-0 w-12 h-16 bg-white border-2 border-stone-400 rounded flex items-center justify-center text-xl text-stone-400 font-bold">B</div>
                    <div className="flex-1 space-y-1.5">
            ...
            <LabeledBox label="Why · 왜 이 책을 읽었나" valKey="read_why" data={data} onChange={onChange} color="bg-slate-50 border-slate-200" textColor="text-slate-800" placeholder="예: 새 습관 만들기에 계속 실패해서 시스템적 접근을 배우려고" />
            ...
            <LabeledBox label="Highlights · 밑줄 친 문장 (인용 그대로)" valKey="read_highlights" data={data} onChange={onChange} color="bg-stone-50 border-stone-200" textColor="text-stone-800" placeholder={'"매일 1%씩 나아지면 1년 후 37배 좋아진다."\n"습관은 정체성에 기반할 때 지속된다."'} />
            <LabeledBox label="Takeaways · 핵심 교훈 3가지 (자기 말로)" valKey="read_takeaways" data={data} onChange={onChange} color="bg-slate-50 border-slate-300" textColor="text-slate-900" placeholder={"1. 결과가 아니라 시스템을 바꿔라\n2. 정체성 기반 습관(\"나는 글 쓰는 사람\")\n3. 환경 디자인이 의지력보다 강하다"} />
            <LabeledBox label="Action · 이번 주 실천 1개" valKey="read_action" data={data} onChange={onChange} color="bg-stone-50 border-stone-300" textColor="text-stone-900" placeholder="예: 책상에 노트 펼쳐두기 (트리거 환경 디자인) → 매일 5분 글쓰기 시도" />
...
function WeeklyJournalGrid({ data, onChange }: { data: FrameworkData; onChange: (key: string, val: string) => void }) {
    return (
        <div className="my-2 space-y-2">
            ...
            <LabeledBox label="Events · 이번 주 있었던 일 (사실)" valKey="wj_events" data={data} onChange={onChange} color="bg-slate-50 border-slate-200" textColor="text-slate-800" placeholder={"- 월: Q2 캠페인 킥오프\n- 화·수: 첫 발표 준비\n- 목: 발표 — 예상보다 호응 ↑\n- 금: 다음 단계 합의\n- 주말: 가족 모임"} />

            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <LabeledBox label="Wins · 잘 된 것" valKey="wj_wins" data={data} onChange={onChange} color="bg-slate-50 border-slate-300" textColor="text-slate-900" placeholder="- 발표 후 첫 클라이언트 미팅 따냄\n- 매일 6시 기상 유지" />
                <LabeledBox label="Lessons · 배운 것" valKey="wj_lessons" data={data} onChange={onChange} color="bg-stone-50 border-stone-200" textColor="text-stone-800" placeholder="- 발표 슬라이드는 3장이 5장보다 강력\n- 회의 직후 저장하는 메모가 진짜 자산" />
            </div>

            <LabeledBox label="Feelings · 느낀 감정 (구체 단어로)" valKey="wj_feelings" data={data} onChange={onChange} color="bg-stone-50 border-stone-200" textColor="text-stone-800" placeholder="예: 화요일 불안 → 목요일 안도감 / 금요일에는 자부심 + 약간의 공허" />
            ...
            <LabeledBox label="Insights · 인사이트·깨달음" valKey="wj_insights" data={data} onChange={onChange} color="bg-stone-50 border-stone-300" textColor="text-stone-900" placeholder='예: "준비 부족" 두려움이 사실은 "완벽하지 않은 모습 노출" 두려움이었다' />

            <LabeledBox label="Next Week Intention · 다음 주 의도" valKey="wj_next" data={data} onChange={onChange} color="bg-slate-50 border-slate-300" textColor="text-slate-900" placeholder={"한 단어: 정착\n핵심: 새 클라이언트 온보딩 \n습관: 매일 30분 운동 회복\n관계: 동료 김OO에게 점심 먼저 제안"} />
        </div>
    );
}
