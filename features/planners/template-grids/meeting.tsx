...
export function OneOnOneGrid({ data, onChange }: { data: FrameworkData; onChange: (key: string, val: string) => void }) {
    return (
        <div className="my-2 space-y-2">
            ...
            <LabeledBox label="Their Agenda · 상대가 먼저 꺼낸 주제" valKey="oto_their_agenda" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-900" placeholder="요즘 어떻게 지내요? — 상대가 가장 말하고 싶은 것부터" />

            {/* Updates */}
            <LabeledBox label="Updates · 진행 상황·성과" valKey="oto_updates" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-800" placeholder="지난 1on1 이후 진척 · 마무리한 일 · 진행 중인 일" />

            {/* Blockers / Concerns */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <LabeledBox label="Blockers · 막힌 것·도움 필요" valKey="oto_blockers" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-neutral-700" placeholder="혼자 풀기 어려운 문제 · 의사결정 필요한 것" />
                <LabeledBox label="Concerns · 고민·우려" valKey="oto_concerns" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-neutral-700" placeholder="팀·프로세스·관계·번아웃 등" />
            </div>

            {/* Feedback 양방향 */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <LabeledBox label="Feedback to them · 내가 줄 피드백" valKey="oto_feedback_to" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-900" placeholder="강점·잘한 일 · 개선 포인트(SBI: 상황·행동·영향)" />
                <LabeledBox label="Feedback from them · 내가 받은 피드백" valKey="oto_feedback_from" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-900" placeholder="상대가 나(매니저·동료)에게 준 피드백" />
            </div>
            ...
            {/* Next */}
            <LabeledBox label="Next · 다음 액션·합의 사항" valKey="oto_next" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-neutral-800" placeholder="- 누가 무엇을 언제까지&#10;- 다음 1on1까지 시도해볼 것" />
        </div>
    );
}
...
export function MeetingGrid({ data, onChange }: { data: FrameworkData; onChange: (key: string, val: string) => void }) {
    return (
        <div className="my-2 space-y-2">
            ...
            {/* 목적 + 안건 */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <LabeledBox label="Objective · 회의 목적" valKey="mtg_objective" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-900" placeholder="이 회의가 끝나면 무엇이 결정/완성되어 있어야 하는가?" />
            ...
            {/* 결정 + 액션 */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <LabeledBox label="Decisions · 결정 사항" valKey="mtg_decisions" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-900" placeholder="- A안 채택&#10;- 예산 1,500만원 확정&#10;- 4/30까지 1차 시안 완성" />
                <LabeledBox label="Action Items · 액션 아이템" valKey="mtg_actions" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-neutral-800" placeholder="- [홍길동] 시안 3안 제작 · 4/30&#10;- [김영희] API 스펙 초안 · 5/2&#10;- [박철수] 컬러 팔레트 확정 · 5/3" />
            </div>

            {/* 보류/리스크 + 다음 회의 */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <LabeledBox label="Parked · 보류·이슈" valKey="mtg_parked" data={data} onChange={onChange} placeholder="이번엔 못 정한 것 · 추가 확인 필요" />
                <LabeledBox label="Next Meeting · 다음 회의" valKey="mtg_next" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-900" placeholder="일시 · 안건 · 사전 준비물" />
            </div>
        </div>
    );
}
...
export function InterviewGrid({ data, onChange }: { data: FrameworkData; onChange: (key: string, val: string) => void }) {
    return (
        <div className="my-2 space-y-2">
            ...
            <LabeledBox label="Past Behavior · 과거에 어떻게 해결했나?" valKey="itv_past" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-900" placeholder="최근에 [문제]를 마주쳤을 때 무엇을 했는지 · 시간·돈을 얼마나 썼는지" />

            {/* Goals + Pains 양립 */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <LabeledBox label="Goals · 목표·하고 있는 것" valKey="itv_goals" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-900" placeholder="요즘 가장 신경 쓰는 일·목표" />
                <LabeledBox label="Pains · 문제·불편" valKey="itv_pains" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-neutral-700" placeholder="가장 짜증나는 순간 · 비효율 · 답답함" />
            </div>
            ...
            {/* Surprises + Insights */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <LabeledBox label="Surprises · 예상 밖의 발견" valKey="itv_surprises" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-800" placeholder="가설과 다른 것 · 의외의 행동·맥락" />
                <LabeledBox label="Insights · 핵심 인사이트" valKey="itv_insights" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-neutral-800" placeholder="이 인터뷰가 우리 의사결정에 주는 메시지 1~2줄" />
            </div>

            {/* Next steps */}
            <LabeledBox label="Next · 다음 단계·검증 항목" valKey="itv_next" data={data} onChange={onChange} color="bg-neutral-50 border-neutral-200" textColor="text-slate-900" placeholder="추가 인터뷰 대상 · 프로토타입 검증 · 다른 가설 테스트" />
        </div>
    );
}
...
export function AarGrid({ data, onChange }: { data: FrameworkData; onChange: (key: string, val: string) => void }) {
    const steps = [
        { key: "aar_planned", label: "계획된 것은 무엇이었나?", hint: "원래 의도·목표·예상 결과", color: "bg-neutral-50 border-neutral-200", text: "text-slate-800",
          ph: "예: Q2 캠페인으로 신규 가입 1,500명 확보 · 4주 진행 · 광고비 800만원" },
        { key: "aar_actual",  label: "실제로 일어난 일은?",     hint: "관찰 가능한 사실만 — 해석 X",   color: "bg-neutral-50 border-neutral-200", text: "text-slate-800",
          ph: "예: 신규 가입 1,180명 (목표의 79%) · 4주차 광고 전환율 급락 · 콘텐츠 1편 누락" },
        { key: "aar_diff",    label: "왜 차이가 났나?",         hint: "원인 분석 — 잘된 것·안 된 것 모두", color: "bg-neutral-50 border-neutral-200", text: "text-neutral-800",
          ph: "잘된 것: 1~2주차 후킹 카피 적중 · 인플 협업 확장\n안 된 것: 3주차 크리에이티브 피로 · 검수 병목" },
        { key: "aar_lessons", label: "배운 것 · 다음에 할 것",  hint: "재현 가능한 교훈으로 일반화",     color: "bg-neutral-50 border-neutral-200", text: "text-slate-900",
        ...
        <div className="my-2 space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                ...
                <div className="rounded-lg p-3 bg-neutral-50 border border-neutral-200">
                    <p className="text-xs font-bold text-neutral-900">Improve · 고칠 것</p>
