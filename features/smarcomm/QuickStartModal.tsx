...
const CATEGORIES = [
  {
    ...
    label: '추천',
    items: [
      { icon: Globe, title: 'GEO/SEO 진단으로 시작하기', desc: 'URL 하나로 AI 검색 노출 상태와 검색 최적화를 동시에 점검하세요.', href: '/smarcomm/dashboard/scan', highlight: true },
      { icon: GitBranch, title: '퍼널 분석으로 이탈 구간 찾기', desc: '방문→진단→가입→유료 전환까지 어디서 고객을 잃고 있는지 파악하세요.', href: '/smarcomm/dashboard/funnel' },
      { icon: Mail, title: '자동 이메일로 재방문 유도하기', desc: '가입 후 이메일 자동화 시나리오를 설정하여 지속적인 관계를 유지하세요.', href: '/smarcomm/dashboard/crm/email' },
      { icon: Bell, title: '푸시 메시지로 점수 변화 알리기', desc: '사이트 점수 변화를 실시간으로 알려 재방문과 참여를 유도하세요.', href: '/smarcomm/dashboard/crm/push' },
      { icon: MessageSquare, title: '카카오 알림톡으로 전환율 높이기', desc: '카카오 알림톡을 활용해 92%+ 열람률로 고객에게 직접 전달하세요.', href: '/smarcomm/dashboard/crm/kakao' },
    ],
  },
  {
    id: 'analysis',
    label: '사이트 분석',
    items: [
      { icon: Globe, title: '첫 사이트 진단하기', desc: '무료 GEO/SEO 통합 진단으로 현재 사이트 상태를 파악하세요.', href: '/smarcomm/dashboard/scan', highlight: true },
      { icon: BarChart3, title: 'SmarComm. Index 리포트 이해하기', desc: '6각형 레이더 차트와 브랜드 성격 분석으로 사이트를 종합 평가합니다.', href: '/smarcomm/dashboard/scan' },
      { icon: Activity, title: '경쟁사 사이트와 비교하기', desc: '경쟁사 URL을 입력하고 점수를 직접 비교해보세요.', href: '/smarcomm/dashboard/scan' },
    ],
  },
  {
    id: 'creative',
    label: '소재 제작',
    items: [
      { icon: Palette, title: 'AI로 광고 카피 생성하기', desc: '제품과 타깃을 입력하면 AI가 네이버 SA, 메타 광고 카피를 자동 생성합니다.', href: '/smarcomm/dashboard/creative', highlight: true },
      { icon: Palette, title: '배너 소재 제작하기', desc: '브랜드 가이드 기반으로 인스타, 페이스북 배너를 자동 제작합니다.', href: '/smarcomm/dashboard/creative' },
      { icon: Activity, title: '소재 A/B 테스트 설계하기', desc: '혜택 강조 vs 긴급성 강조 — 데이터로 더 나은 소재를 선택하세요.', href: '/smarcomm/dashboard/abtest' },
    ],
  },
  {
    id: 'data',
    label: '데이터 분석',
    items: [
      { icon: GitBranch, title: '전환 퍼널 분석하기', desc: '방문→진단→가입→유료 전환 각 단계의 이탈률을 분석합니다.', href: '/smarcomm/dashboard/funnel', highlight: true },
      { icon: Users, title: '코호트별 리텐션 확인하기', desc: '주차별 재방문율 히트맵으로 고객 유지력을 측정하세요.', href: '/smarcomm/dashboard/cohort' },
      { icon: Route, title: '고객 여정 타임라인 추적하기', desc: '개별 고객이 SmarComm과 함께한 여정을 한눈에 확인합니다.', href: '/smarcomm/dashboard/journey' },
      { icon: FileBarChart, title: '월간 캠페인 보고서 확인하기', desc: '채널별 성과, ROAS, CPA를 통합 리포트로 분석합니다.', href: '/smarcomm/dashboard/reports' },
    ],
  },
  {
    id: 'crm',
    label: 'CRM 마케팅',
    items: [
      { icon: Mail, title: '환영 이메일 자동화 설정하기', desc: '가입 즉시 환영 이메일 → 7일 후 재방문 유도까지 자동 시나리오를 설정하세요.', href: '/smarcomm/dashboard/crm/email', highlight: true },
      { icon: Bell, title: '점수 변화 푸시 알림 보내기', desc: '사이트 점수가 변하면 고객에게 자동 알림을 보내 재방문을 유도합니다.', href: '/smarcomm/dashboard/crm/push' },
      { icon: MessageSquare, title: '카카오 알림톡 연동하기', desc: '진단 완료, 미팅 예약 등 핵심 알림을 카카오로 전달하세요.', href: '/smarcomm/dashboard/crm/kakao' },
      { icon: Users, title: 'CRM 파이프라인 관리하기', desc: '리드→미팅→진행→계약, 고객 상태를 한눈에 관리하세요.', href: '/smarcomm/dashboard/crm' },
    ],
  },
];
      ...
      <div className="relative mx-4 flex h-[560px] w-full max-w-4xl overflow-hidden rounded-2xl border border-border bg-white shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="hidden w-64 shrink-0 border-l border-border bg-surface p-6 lg:block">
          <div className="rounded-xl bg-white p-4 shadow-sm">
            ...
            <button
              onClick={() => { router.push(category.items[0]?.href || '/smarcomm/dashboard'); onClose(); }}
