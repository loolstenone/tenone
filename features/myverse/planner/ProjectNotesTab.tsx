...
// myverse:canvas= 신규 포맷 · planners:canvas= 레거시 호환
const CANVAS_MARKER_RE = /^<!--\s*(?:myverse|planners):canvas=([a-zA-Z0-9_-]+)\s*-->/;
...
function buildCanvasContent(canvasId: string): string {
    return `<!-- myverse:canvas=${canvasId} -->`;
}
...
export function ProjectNotesTab({ projectId, projectCategory }: { projectId: string; projectCategory?: string | null }) {
    ...
    async function addBlankNote() {
        setSaving(true);
        try {
            const idx = notes.filter(n => !isHandwritingContent(n.content) && !(n.content ?? "").includes("planners-template")).length + 1;
                ...
                <button
                    onClick={async () => {
                        const res = await fetch(`/api/myverse/projects/${projectId}/notes`, {
                            ...
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({
                                title: `손글씨 ${notes.filter(n => isHandwritingContent(n.content)).length + 1}`,
