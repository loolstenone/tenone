...
/**
 * CanvasEditor — 캔버스 엔진 풀스크린 래퍼.
 *
 * 스트로크 + 도형 + 지우개 + 팬/줌 + 선택/리사이즈/올가미.
 * 1.5s 디바운스 자동 저장 + 언마운트 시 즉시 flush.
 * 손바닥 거부(PalmRejection) + RAF 라이브 렌더링.
 */
...
export default function CanvasEditor({ initialDoc, onSave, className, exportFilename = "canvas" }: CanvasEditorProps) {
    ...
    useEffect(() => {
        ...
        return () => {
            unsubChange();
            // 언마운트 직전 대기 중인 저장이 있으면 즉시 flush — 모달 빠른 닫기 시 데이터 유실 방지
            if (saveTimerRef.current) {
                ...
                if (onSave) {
                    try {
                        ...
                        onSave(engine.serialize());
                    } catch (err) {
                        console.error("CanvasEditor flush save 실패", err);
                    }
                }
            }
            engine.destroy();
