...
import {
    applyMyverseTheme,
    applyMyverseFont,
    applyMyverseUserFont,
    applyMyverseThemeMode,
    applyMyverseRadius,
    type MyverseThemeMode,
    type MyverseRadius,
} from "@/features/myverse/app/MyverseThemeProvider";
import {
...
const RADIUS_THEMES: { key: MyverseRadius; label: string; desc: string; preview: string }[] = [
...
export function SettingsTheme({ save: _save, showToast: _showToast }: Props) {
    ...
    const [radiusTheme, setRadiusTheme]       = useState<MyverseRadius>("soft");
    const [fontFamily, setFontFamily]         = useState("sans");
    const [themeMode, setThemeMode]           = useState<MyverseThemeMode>("system");
    ...
    useEffect(() => {
        ...
        const savedMode   = localStorage.getItem("myverse_theme_mode") as MyverseThemeMode | null;
        const savedRadius = localStorage.getItem("myverse_radius_theme") as MyverseRadius | null;
    ...
    function applyTheme(key: string) {
        ...
        applyMyverseTheme(key);
    }

    function applyMode(mode: MyverseThemeMode) {
        setThemeMode(mode);
    ...
    function applyPreset(p: SettingsPreset) {
        ...
        applyMyverseTheme(p.color);
        applyMyverseRadius(p.radius);
        applyMyverseFont(p.font);
        applyMyverseUserFont(p.userFont);
        applyMyverseThemeMode(p.mode);
