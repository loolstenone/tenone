...
export function SettingsLayout({ children }: Props) {
    ...
    return (
        <div className="pp-settings max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6 md:py-10" ref={containerRef}>
            {/* ── 모바일 sticky 탭 row ── */}
            <nav
                className="sticky top-12 z-30 -mx-4 md:-mx-6 px-4 md:px-6 backdrop-blur mb-5"
            ...
            {/* ── 본문 grid ──
                  기본: 1열 (main)
                  xl(1280+): 2열 (main + Live Preview)
                  items-start로 preview가 콘텐츠 높이만 차지 (sticky 정상 작동) */}
            <div className="xl:grid xl:grid-cols-[1fr_380px] xl:gap-8 xl:items-start">
                {/* main content */}
...
export function GroupMarker({ group, label, no }: { group: SettingsGroup["key"]; label: string; no: string }) {
    return (
        ...
        >
            {/* IntersectionObserver는 wrapper 자체를 관찰. 시각 라벨은 pill nav가 있으므로 숨김 처리 */}
            <p className="pp-eyebrow opacity-0 select-none pointer-events-none">{label}</p>
        </div>
    );
}
