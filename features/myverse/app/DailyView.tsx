...
import type { MyverseDaily, MyverseTask } from "@/lib/myverse/types";
...
export function DailyView({ initialDate, autoCompose }: { initialDate: string; autoCompose?: boolean }) {
    ...
    async function flushOfflineQueue() {
        try {
            ...
            if (!raw) return;
            const queue: Array<{ date: string; patch: Partial<MyverseDaily> }> = JSON.parse(raw);
    ...
    async function save(patch: Partial<MyverseDaily>) {
        if (!isOnline) {
            try {
                const raw = localStorage.getItem(offlineQueueKey);
    ...
    async function importSelectedPending() {
        ...
        try {
            ...
            // 1. 오늘 tasks에 추가
            const newTasks: MyverseTask[] = toImport.map(t => ({
                id: `t_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
                text: t.text,
                status: "todo" as const,
                time: null,
                project_id: null,
                priority: (t.priority as MyverseTask["priority"]) ?? null,
                memo: null,
            }));
            ...
            await Promise.all(
                Array.from(sourceUpdates.entries()).map(async ([srcDate, selectedIds]) => {
                    ...
                    const updated = (d.daily.tasks as MyverseTask[]).map(t =>
    ...
    function updateTaskPriority(taskId: string, priority: MyverseTask["priority"]) {
    ...
    async function moveTask(taskId: string, newDate: string, newTime?: string | null) {
        ...
        try {
            ...
            const targetTasks: MyverseTask[] = Array.isArray(d.daily?.tasks) ? d.daily.tasks : [];
            const newTask: MyverseTask = {
                ...original,
                id: `t_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
                status: 'todo',
                time: normalizedTime,
...
interface TaskRowProps {
    ...
    onPriorityChange?: (p: MyverseTask["priority"]) => void;
...
function PriorityPicker({ value, onChange }: { value: MyverseTask["priority"]; onChange: (p: MyverseTask["priority"]) => void }) {
