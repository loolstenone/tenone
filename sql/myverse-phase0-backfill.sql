...
UPDATE myverse_daily_routines SET domain = CASE category
    WHEN 'work'      THEN 'work'
...
UPDATE myverse_daily_places SET domain = CASE category
    WHEN 'work'     THEN 'work'
    WHEN 'meal'     THEN 'body'
    WHEN 'exercise' THEN 'body'
...
UPDATE myverse_calendar_entries SET domain = CASE kind
...
UPDATE myverse_projects SET domain = 'work'
    WHERE domain = 'daily' OR domain IS NULL;

-- 5) contacts: 모두 relation 도메인
UPDATE myverse_contacts SET domain = 'relation'
    WHERE domain = 'daily' OR domain IS NULL;
...
UPDATE myverse_daily_moments SET capture_mode = 'active' WHERE capture_mode IS NULL OR capture_mode = '';
UPDATE myverse_daily_routines SET capture_mode = 'active' WHERE capture_mode IS NULL OR capture_mode = '';
UPDATE myverse_daily_places SET capture_mode = 'active' WHERE capture_mode IS NULL OR capture_mode = '';
UPDATE myverse_calendar_entries SET capture_mode = 'active' WHERE capture_mode IS NULL OR capture_mode = '';
