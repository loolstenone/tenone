...
ALTER TABLE planners_identities
  ADD COLUMN IF NOT EXISTS resume JSONB DEFAULT '{}';

-- planners_users.activity_bases — 사용자의 활동 거점 목록
...
COMMENT ON COLUMN planners_identities.resume IS '이력서 데이터 (학력·경력·자격증·기술·언어 JSON)';
COMMENT ON COLUMN planners_users.activity_bases IS '활동 거점 목록 (시간 트래커·캘린더 자동 라벨링용)';
