-- myverse_yearly.anniversaries (jsonb 배열) → myverse_calendar_entries 일괄 이전
-- 중복 방지: 같은 member + 같은 제목 + 같은 날짜가 이미 있으면 스킵

INSERT INTO myverse_calendar_entries (
...
SELECT
    ...
    END AS recurrence
FROM myverse_yearly y, jsonb_array_elements(y.anniversaries) AS a
WHERE jsonb_typeof(y.anniversaries) = 'array'
  AND (a->>'label') IS NOT NULL
  AND (a->>'date') ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
  AND NOT EXISTS (
      SELECT 1 FROM myverse_calendar_entries e
