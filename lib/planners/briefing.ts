...
export async function gatherContext(memberId: string, date: string): Promise<BriefingContext> {
    ...
    const [member, plannerUser, identity, todayDaily, yestDaily, weekly, monthly, projects] = await Promise.all([
        ...
        admin.from("planners_weekly").select("vrief_what, vrief_why, vrief_how, gpr_goal, gpr_plan").eq("member_id", memberId).eq("year", year).eq("week", week).maybeSingle(),
        admin.from("planners_monthly").select("theme, focus_areas").eq("member_id", memberId).eq("year", yearNum).eq("month", month).maybeSingle(),
        admin.from("planners_projects").select("title, end_date, planners_project_gprs(progress)").eq("member_id", memberId).eq("status", "active"),
    ]);
    ...
    return {
        ...
        monthly: monthly.data
            ...
            : undefined,
        projects:
            (projects.data ?? []).map((p: { title: string; end_date: string | null; planners_project_gprs: Array<{ progress: number }> | { progress: number } | null }) => ({
                title: p.title,
                end_date: p.end_date,
                progress: Array.isArray(p.planners_project_gprs)
                    ? p.planners_project_gprs[0]?.progress ?? 0
                    : p.planners_project_gprs?.progress ?? 0,
            })) ?? [],
