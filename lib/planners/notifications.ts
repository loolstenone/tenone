...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;padding:32px 24px;">
  <tr><td>
    ...
    <p style="margin-top:24px;font-size:11px;color:#9CA3AF;text-align:center;line-height:1.6;">
      <a href="https://planners.tenone.biz/planners/app" style="color:#0F766E;text-decoration:none;">앱에서 보기</a>
      &nbsp;·&nbsp;
      <a href="https://planners.tenone.biz/planners/app/settings" style="color:#6B7280;text-decoration:none;">알림 설정</a>
    </p>
...
export async function sendPushBriefing(params: {
    ...
    const { data: subs } = await admin
        .from("planners_push_subscriptions")
        .select("*")
            ...
            try {
                await webpush.sendNotification(
                    ...
                    JSON.stringify({
                        title: `[PP AI] ${title}`,
                        body,
                        url: "/planners/app/ai-briefing",
                    })
                );
                success++;
                admin.from("planners_push_subscriptions").update({ last_used_at: new Date().toISOString() }).eq("id", s.id);
            } catch (e) {
                const err = e as { statusCode?: number };
                if (err.statusCode === 410 || err.statusCode === 404) {
                    await admin.from("planners_push_subscriptions").delete().eq("id", s.id);
                }
            }
        })
    );
