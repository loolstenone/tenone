...
export interface EEATBreakdown {
    ...
    authoritativeness: number | null; // 0~100 or null(N/A) — 백링크·DR (Phase 4까지 N/A)
...
export function computeIndex(result: AnalysisResult): IndexBreakdown {
    // ── Findability 점수 ──
    ...
    const citability = Math.min(100, platformScore + readinessScore + techScore);
    ...
    // Authoritativeness: Phase 4 외부 도구(Ahrefs/Moz) 필요 → 현재 N/A
    const authoritativeness: number | null = null;
