 ...
 * 심화: CH Deep C맞춤 + AP Deep C맞춤 (scoreChDeepB/scoreApDeepB 재사용)
 */

// CH/AP Deep 채점은 B와 동일 구조 (character_deep/aptitude_deep 모듈 필터)
import { scoreChDeepB, scoreApDeepB } from './scoring-b';
export { scoreChDeepB as scoreChDeepC, scoreApDeepB as scoreApDeepC } from './scoring-b';
export type { CHDeepBSubscale as CHDeepCSubscale, HollandType } from './data/b-deep-questions';
...
export interface HitCScoreResult {
  ...
  ctype: CType;
  fakingFlag: boolean;
  // CH Deep C맞춤 / AP Deep C맞춤 (심화 레이어 응답 있을 때)
  chDeepScores?: {
    scores: Record<string, number>;
...
export function scoreHitC(responses: ResponseRow[]): HitCScoreResult {
  ...
  // CH Deep C / AP Deep C — 심화 응답 있을 때만
