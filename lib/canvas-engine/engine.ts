...
export class CanvasEngine {
    ...
    updateElementsBatch(patches: { id: string; patch: Partial<CanvasElement> }[]): void {
        if (patches.length === 0) return;
        this.history.push(this.doc);
        const patchMap = new Map(patches.map(p => [p.id, p.patch]));
        const next = this.doc.elements.map(el => {
            const patch = patchMap.get(el.id);
            return patch ? ({ ...el, ...patch, updatedAt: Date.now() } as CanvasElement) : el;
        });
        this.doc = { ...this.doc, elements: next };
        this.emit("change", this.doc);
    }
